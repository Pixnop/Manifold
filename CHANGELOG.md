...
### Changed
- **Custom dimensions are now actually lit.** Because of the dim-0 relight bug above, the `WithRelightHeight` band never lit the dimension it belonged to - many custom dims rendered dark and modders compensated (or attributed it to depth). With the fix, sky-exposed content inside the relight band carries proper sunlight. Note: the engine renders sun-lit blocks in custom dimensions at full brightness regardless of the time of day (no public per-dim day/night hook); real per-dimension nights are tracked in #55.

### Added
- **`IDimensionBuilder.WithDarkSky(int ceilingY)`** - makes a dimension dark by sealing every generated column with an opaque ceiling at `ceilingY`. Vintage Story floods skylight downward from the top of a dimension's column with no per-dimension gate, so an open / mostly-air custom dimension renders fully lit regardless of time of day (an engine limitation, see #62); the opaque cap stops that flood so the area below stays dark and is lit only by block light (torches, lamps). Capping every generated column also makes those chunks non-empty, suppressing a client-side full-bright bleed from neighbouring empty chunks. Manifold raises the relight band to `ceilingY + 1` automatically. Best for enclosed / underground dimensions; solid-filled dimensions are already dark without it. Verified in-game (dark interior, torches light it). Demonstrated by the sample's new `/darkdim`. Addresses the actionable half of #55 / #62.
